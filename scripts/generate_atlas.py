...
# ---------------------------------------------------------------------------
# Academic Data Visualization color palette
# ---------------------------------------------------------------------------
CNS_COLORS = {
    "green":   "#009E73",
    "purple":  "#CC79A7",
...
def style_ax(ax, hide_spines=("top", "right")):
    """应用统一低饱和轴样式：弱化结构、突出数据层。"""
    for spine in hide_spines:
        ax.spines[spine].set_visible(False)
    for spine in ax.spines.values():
        spine.set_color(CNS_COLORS["dark_gray"])
        spine.set_linewidth(0.55)
    ax.tick_params(colors=CNS_COLORS["dark_gray"], width=0.5, length=2)
...
# ---------------------------------------------------------------------------
# Atlas 6: Domain / Work Patterns (16 variants)
# ---------------------------------------------------------------------------

def _kaplan_meier(ax):
    """a) Kaplan–Meier survival curves with censoring marks."""
    time = np.array([0, 1, 2, 3, 5, 7, 9, 11])
    control = np.array([1.0, .93, .84, .76, .64, .52, .43, .37])
    treatment = np.array([1.0, .97, .92, .87, .79, .72, .66, .61])
    ax.step(time, control, where="post", color=CNS_COLORS["blue"], linewidth=1.1)
    ax.step(time, treatment, where="post", color=CNS_COLORS["green"], linewidth=1.1)
    ax.plot([4, 8], [0.76, 0.46], "+", color=CNS_COLORS["blue"], markersize=5, mew=.7)
    ax.plot([6, 10], [0.74, 0.64], "+", color=CNS_COLORS["green"], markersize=5, mew=.7)
    ax.set_xlim(0, 11)
    ax.set_ylim(.25, 1.04)
    ax.set_xticks([])
    ax.set_yticks([])
...
def _dumbbell(ax):
    """b) Dumbbell comparison for paired categories."""
    before = np.array([2.1, 3.7, 4.5, 5.1, 6.2, 7.4])
    after = np.array([3.2, 3.3, 5.4, 4.8, 7.1, 6.6])
    y = np.arange(len(before))
    for yi, left, right in zip(y, before, after):
        ax.plot([left, right], [yi, yi], color=CNS_COLORS["light_gray"], linewidth=1.4)
    ax.scatter(before, y, s=20, color=CNS_COLORS["blue"], edgecolors="white", linewidth=.25, zorder=2)
    ax.scatter(after, y, s=20, color=CNS_COLORS["red"], edgecolors="white", linewidth=.25, zorder=2)
    ax.set_xticks([])
    ax.set_yticks([])
    style_ax(ax)
    add_panel_label_inside(ax, LABELS[1])
...
def _slopegraph(ax):
    """c) Slopegraph for two ordered states."""
    left = np.array([.18, .31, .46, .58, .73])
    right = np.array([.22, .49, .39, .76, .64])
    for index, (start, end) in enumerate(zip(left, right)):
        color = CNS_COLORS["green"] if end >= start else CNS_COLORS["red"]
        ax.plot([0, 1], [start, end], color=color, linewidth=.9, alpha=.85)
        ax.scatter([0, 1], [start, end], s=10, color=color, edgecolors="white", linewidth=.2)
    ax.set_xlim(-.12, 1.12)
    ax.set_ylim(.08, .86)
    ax.set_xticks([])
    ax.set_yticks([])
...
def _pareto(ax):
    """d) Pareto bars with a cumulative-percentage line."""
    values = np.array([31, 23, 17, 12, 8, 5, 3])
    cumulative = values.cumsum() / values.sum() * 100
    x = np.arange(len(values))
    ax.bar(x, values, color=CNS_COLORS["blue"], edgecolor="white", linewidth=.2)
    twin = ax.twinx()
    twin.plot(x, cumulative, "o-", color=CNS_COLORS["red"], linewidth=.8, markersize=2.4)
    twin.set_ylim(0, 108)
    twin.set_yticks([])
...
def _control_chart(ax):
    """e) Control chart with centre line and three-sigma bounds."""
    x = np.arange(1, 21)
    y = 10 + np.array([-.4, .1, -.2, .3, .5, -.1, .2, -.5, .0, .4, .1, .7, -.2, .3, 1.8, -.1, .2, -.3, .1, .0])
    ax.plot(x, y, "o-", color=CNS_COLORS["blue"], linewidth=.8, markersize=2.3)
    ax.axhline(10, color=CNS_COLORS["dark_gray"], linewidth=.45)
    ax.axhline(11.2, color=CNS_COLORS["red"], linewidth=.45, linestyle="--")
    ax.axhline(8.8, color=CNS_COLORS["red"], linewidth=.45, linestyle="--")
    ax.scatter([15], [y[14]], color=CNS_COLORS["red"], s=18, zorder=3)
    ax.set_xticks([])
    ax.set_yticks([])
    style_ax(ax)
    add_panel_label_inside(ax, LABELS[4])
...
def _gantt_chart(ax):
    """f) Gantt chart for project schedules and milestones."""
    starts = [0, 1.2, 2.2, 3.8, 5.0]
    durations = [3.6, 2.4, 3.1, 1.8, 2.6]
    y = np.arange(5)[::-1]
    for index, (start, duration, yi) in enumerate(zip(starts, durations, y)):
        ax.barh(yi, duration, left=start, height=.46, color=CNS_PALETTE[index], edgecolor="white", linewidth=.2)
    ax.axvline(4.5, color=CNS_COLORS["red"], linewidth=.55, linestyle="--")
    ax.set_xticks([])
    ax.set_yticks([])
    style_ax(ax)
    add_panel_label_inside(ax, LABELS[5])
...
def _stage_funnel(ax):
    """g) Funnel chart for stage conversion, not data-flow Sankey."""
    widths = [5.6, 4.6, 3.4, 2.1, 1.1]
    colors = [CNS_COLORS["blue"], CNS_COLORS["teal"], CNS_COLORS["green"], CNS_COLORS["orange"], CNS_COLORS["red"]]
    for index, (width, color) in enumerate(zip(widths, colors)):
        y0 = len(widths) - 1 - index
        ax.fill_between([-width / 2, width / 2], y0, y0 + .82, color=color, alpha=.88, linewidth=0)
    ax.set_xlim(-3.2, 3.2)
    ax.set_ylim(-.1, 5.1)
    ax.set_xticks([])
    ax.set_yticks([])
...
def _treemap(ax):
    """h) Treemap with manually specified hierarchy proportions."""
    rectangles = [
        (0, 0, .54, .62, CNS_COLORS["blue"]), (.55, 0, .45, .38, CNS_COLORS["green"]),
        (.55, .39, .27, .61, CNS_COLORS["orange"]), (.83, .39, .17, .34, CNS_COLORS["red"]),
        (.83, .74, .17, .26, CNS_COLORS["purple"]),
    ]
    for x0, y0, width, height, color in rectangles:
        ax.add_patch(plt.Rectangle((x0, y0), width, height, facecolor=color, edgecolor="white", linewidth=1.1))
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ax.set_xticks([])
    ax.set_yticks([])
...
def _calendar_heatmap(ax):
    """i) Calendar-style heatmap for repeated daily or batch signals."""
    values = np.random.RandomState(710).uniform(.05, 1, size=(7, 12))
    ax.imshow(values, aspect="auto", cmap=CNS_SEQUENTIAL_CMAP, vmin=0, vmax=1)
    ax.set_xticks([])
    ax.set_yticks([])
...
def _network_graph(ax):
    """j) Quantitative network with visible node and edge encodings."""
    coords = np.array([[0, .9], [.85, .38], [.52, -.62], [-.52, -.62], [-.85, .38], [0, .05]])
    edges = [(0, 1, .9), (1, 2, .55), (2, 3, .8), (3, 4, .45), (4, 0, .7), (0, 5, .5), (1, 5, .8), (3, 5, .65)]
    for first, second, weight in edges:
        ax.plot(coords[[first, second], 0], coords[[first, second], 1], color=CNS_COLORS["gray"], linewidth=.3 + weight, alpha=.65)
    sizes = [38, 28, 25, 32, 20, 48]
    ax.scatter(coords[:, 0], coords[:, 1], s=sizes, color=CNS_PALETTE, edgecolors="white", linewidth=.5, zorder=2)
...
def _spatial_bubble(ax):
    """k) Spatial bubble map using coordinates and a quantitative size channel."""
    rng = np.random.RandomState(711)
    points = rng.uniform(0, 1, size=(38, 2))
    magnitude = rng.gamma(shape=2.0, scale=7, size=38)
    ax.scatter(points[:, 0], points[:, 1], s=magnitude * 2.2, c=points[:, 1], cmap=CNS_SEQUENTIAL_CMAP,
               alpha=.75, edgecolors="white", linewidth=.25)
    ax.set_xlim(-.05, 1.05)
    ax.set_ylim(-.05, 1.05)
    ax.set_xticks([])
    ax.set_yticks([])
    for spine in ax.spines.values():
        spine.set_color(CNS_COLORS["light_gray"])
    add_panel_label_inside(ax, LABELS[10])


def _dendrogram_plot(ax):
    """l) Dendrogram for hierarchical clustering or phylogenetic structure."""
    data = np.random.RandomState(712).normal(size=(12, 4))
    dendrogram(linkage(data, method="ward"), ax=ax, no_labels=True, color_threshold=0,
               above_threshold_color=CNS_COLORS["blue"])
    ax.set_xticks([])
    ax.set_yticks([])
    style_ax(ax)
    add_panel_label_inside(ax, LABELS[11])
...
def _genome_lollipop(ax):
    """m) Lollipop mutation plot with exons and variant positions."""
    ax.hlines(0, 0, 10, color=CNS_COLORS["dark_gray"], linewidth=1)
    for start, width in [(0.4, 1.3), (2.3, 1.0), (4.0, 1.8), (6.4, 1.2), (8.2, 1.1)]:
        ax.add_patch(plt.Rectangle((start, -.18), width, .36, facecolor=CNS_COLORS["blue"], edgecolor="white", linewidth=.3))
    positions = [1.0, 2.8, 3.1, 4.7, 5.2, 6.9, 8.6, 9.0]
    heights = [.55, .8, .42, 1.0, .65, .5, .9, .38]
    for index, (position, height) in enumerate(zip(positions, heights)):
        color = CNS_COLORS["red"] if index in (3, 6) else CNS_COLORS["green"]
        ax.vlines(position, .18, height, color=color, linewidth=.7)
        ax.scatter([position], [height], s=18, color=color, edgecolors="white", linewidth=.25, zorder=2)
    ax.set_xlim(0, 10)
    ax.set_ylim(-.4, 1.2)
    ax.set_xticks([])
    ax.set_yticks([])
...
def _shap_beeswarm(ax):
    """n) SHAP-style beeswarm, preserving feature rank and local spread."""
    rng = np.random.RandomState(713)
    for index in range(6):
        values = rng.normal(loc=.35 - index * .12, scale=.42 - index * .025, size=42)
        jitter = rng.uniform(-.22, .22, size=len(values))
        color_values = rng.uniform(0, 1, size=len(values))
        ax.scatter(values, np.full(len(values), 5 - index) + jitter, s=6, c=color_values,
                   cmap=CNS_SEQUENTIAL_CMAP, edgecolors="none", alpha=.75)
    ax.axvline(0, color=CNS_COLORS["dark_gray"], linewidth=.45)
    ax.set_xticks([])
    ax.set_yticks([])
    style_ax(ax)
    add_panel_label_inside(ax, LABELS[13])
...
def _coefficient_plot(ax):
    """o) Dot-whisker coefficient plot with a neutral reference line."""
    estimates = np.array([-.42, -.22, -.05, .16, .31, .48])
    errors = np.array([.12, .09, .11, .08, .14, .10])
    y = np.arange(len(estimates))[::-1]
    ax.axvline(0, color=CNS_COLORS["dark_gray"], linewidth=.5)
    ax.errorbar(estimates, y, xerr=errors, fmt="none", color=CNS_COLORS["gray"], linewidth=.8, capsize=1.8)
    ax.scatter(estimates, y, s=22, c=[CNS_COLORS["blue"] if value < 0 else CNS_COLORS["red"] for value in estimates],
...
def _ternary_plot(ax):
    """p) Ternary composition plot for three parts summing to one."""
    vertices = np.array([[0, 0], [1, 0], [.5, np.sqrt(3) / 2]])
    ax.plot(np.r_[vertices[:, 0], vertices[0, 0]], np.r_[vertices[:, 1], vertices[0, 1]],
            color=CNS_COLORS["dark_gray"], linewidth=.6)
    composition = np.random.RandomState(714).dirichlet([2.4, 1.8, 2.1], size=55)
    coords = composition @ vertices
    ax.scatter(coords[:, 0], coords[:, 1], s=8, c=composition[:, 2], cmap=CNS_SEQUENTIAL_CMAP,
               edgecolors="white", linewidth=.15, alpha=.75)
    ax.set_xlim(-.08, 1.08)
    ax.set_ylim(-.08, .98)
    ax.set_xticks([])
    ax.set_yticks([])
...
DOMAIN_WORK_PANELS = [
    _kaplan_meier, _dumbbell, _slopegraph, _pareto,
    _control_chart, _gantt_chart, _stage_funnel, _treemap,
    _calendar_heatmap, _network_graph, _spatial_bubble, _dendrogram_plot,
    _genome_lollipop, _shap_beeswarm, _coefficient_plot, _ternary_plot,
]
...
def main():
    ...
    print(f"\nAll {len(atlases)} atlas images generated successfully.")

...
def check_ap0_style_baseline(source: str) -> list[Finding]:
    ...
    # Color baseline
    color_patterns = ['CATEGORICAL = [', 'DIVERGING = [', '#0072B2', '#D55E00', '#1A1A1A']
    ...
    findings.append(Finding("AP-0", color_ok,
        ...
        "Missing Nature-ready accessible Academic Data Visualization palette (CATEGORICAL/DIVERGING)"))

    # Export baseline
    export_patterns = ["pdf.fonttype", "svg.fonttype", "save_nature_ready_figure"]
    export_ok = all(p in source for p in export_patterns) and (
        "42" in source or "'42'" in source or '"42"' in source
    )
...
def check_ap5_low_res_export(source: str) -> Finding:
    """Verify that an editable vector export is present."""

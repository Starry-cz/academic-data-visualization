...
# 默认视觉系统：采用色盲安全的语义配色，避免只靠红绿区分类别。
# 六个主色分别承担基线、次级组、对比组、第四类别、强调与辅助类别的语义。
CATEGORICAL = ["#0072B2", "#009E73", "#E69F00", "#CC79A7", "#D55E00", "#56B4E9"]
CATEGORICAL_EXTENDED = [
    "#0072B2", "#009E73", "#E69F00", "#CC79A7", "#D55E00", "#56B4E9",
    "#999999", "#332288", "#88CCEE", "#44AA99", "#DDCC77", "#AA4499",
]
...
JOURNAL_PALETTES = {
    # Nature-family：沿用可访问语义色，确保跨面板和灰度打印时仍可辨识。
    "nature": {
        "CATEGORICAL": ["#0072B2", "#009E73", "#E69F00", "#CC79A7", "#D55E00", "#56B4E9"],
        "CATEGORICAL_EXTENDED": [
            "#0072B2", "#009E73", "#E69F00", "#CC79A7", "#D55E00", "#56B4E9",
    ...
    # Cell-family: slightly warmer but never turns the full panel into an accent block.
    "cell": {
        "CATEGORICAL": ["#7698AA", "#B5CCB2", "#F1C37E", "#C0ACC9", "#D98764", "#91AFAC"],
        "CATEGORICAL_EXTENDED": [
            "#7698AA", "#B5CCB2", "#F1C37E", "#C0ACC9", "#D98764", "#91AFAC",
            "#C2D7DF", "#E0ECDD", "#F8E7BB", "#E0D5E5", "#EFC5B4", "#CBD9D5",
        ],
        "DIVERGING": ["#6D8FA0", "#FBF8F3", "#D98764"],
        "SEQUENTIAL": ["#FBF8F2", "#F1C37E", "#D98764"],
        "ACCENT": "#D98764",
        "GREY": "#8A989C",
    },
    # Science-family: the most restrained variant, suitable for dense quantitative panels.
    "science": {
        "CATEGORICAL": ["#718B9A", "#B6C7B8", "#E7C889", "#B9B2C4", "#C98B78", "#97AAAC"],
        "CATEGORICAL_EXTENDED": [
            "#718B9A", "#B6C7B8", "#E7C889", "#B9B2C4", "#C98B78", "#97AAAC",
            "#BED0D7", "#DDE7DE", "#F3E1B6", "#D8D1E0", "#E4BDAF", "#CAD6D5",
        ],
        "DIVERGING": ["#627D8C", "#F7F8F6", "#BE887A"],
        "SEQUENTIAL": ["#F5F7F6", "#C7D4D5", "#718B9A"],
        "ACCENT": "#C98B78",
        "GREY": "#8A989C",
    },
}
...
PANEL_ASPECT = {
    ...
    "scatter": 0.75,
    "bubble_scatter": 0.75,
    "correlation_bubble_matrix": 1.0,
    "correlation_network": 1.0,
    "geographic_bubble_map": 0.58,
    "ridge": 0.65,
    "kde": 0.65,
    "density": 0.65,
    "sankey": 0.50,
...
def _unified_rcparams(n_panels=4):
    ...
    return {
        ...
        **fonts,
        ...
        "grid.color": NEUTRAL["grid"], "grid.linewidth": 0.55,
        "axes.axisbelow": True, "legend.frameon": False,
        "pdf.fonttype": 42, "svg.fonttype": "none",
        "savefig.bbox": "tight", "savefig.dpi": 450,
    }
...
def panel_specs(panel_types: Iterable[str], fig_width_mm: float = 183):
    ...
    for i, _panel_type in enumerate(panel_types):
        r = i // cols
        specs.append({"width_mm": panel_w_mm, "height_mm": panel_w_mm * row_heights[r], "dpi": 450})
...
def _make_grid(n: int, panel_types: list[str], fig_width_mm: float, hero_idx: int | None):
    ...
    if hero_idx is not None and 0 <= hero_idx < n and 3 <= n <= 6:
        ...
        for i in range(n):
            if i == hero_idx:
                specs.append({"width_mm": panel_w_mm * width_ratios[0], "height_mm": panel_w_mm * rows, "dpi": 450})
            else:
                specs.append({"width_mm": panel_w_mm, "height_mm": panel_w_mm, "dpi": 450})
...
def compose_figure(
    ...
    save_nature_ready_figure(fig, output_prefix)
...
def r_png_device(spec: dict, output_path: str) -> str:
    ...
    Generated call example for 65mm panel:
        png("panel.png", width=2.559, height=2.559, units="in", res=450)
...
def _panel_label(ax, text: str, dark_bg: bool = False):
    ...
    if dark_bg:
        ...
        ax.text(0.02, 0.96, text, transform=ax.transAxes,
                fontsize=8, fontweight="bold", va="top", color="#1A1A1A")

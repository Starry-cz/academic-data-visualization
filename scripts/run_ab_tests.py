...
def run_all():
    ...
    s1_academic_data_visualization_passes = []
    s1_academic_data_visualization_details = []

    # Check if PCA R script can run
    pca_r = REPO_ROOT / "assets" / "figures" / "PCA" / "plot_PCA.R"
    s1_academic_data_visualization_passes.append(pca_r.exists())
    s1_academic_data_visualization_details.append(f"Asset exists: {pca_r.exists()}")

    # Check compose.py has r_png_device with type="cairo"
    compose_py = REPO_ROOT / "scripts" / "compose.py"
    with open(compose_py, encoding="utf-8", errors="replace") as f: py_src = f.read()
    s1_academic_data_visualization_passes.append('type="cairo"' in py_src or "type='cairo'" in py_src)
    s1_academic_data_visualization_details.append("PNG cairo rule in compose.py")

    # 检查共享的 Nature-ready 色盲安全语义色板。
    color_md = REPO_ROOT / "references" / "color-palettes.md"
    with open(color_md, encoding="utf-8", errors="replace") as f: color_src = f.read()
    s1_academic_data_visualization_passes.append("#0072B2" in color_src and "#D55E00" in color_src)
    s1_academic_data_visualization_details.append("Accessible palette in color-palettes.md")

    # Check typography baseline includes Arial
    typo_md = REPO_ROOT / "references" / "typography.md"
    with open(typo_md, encoding="utf-8", errors="replace") as f: typo_src = f.read()
    s1_academic_data_visualization_passes.append("Arial" in typo_src)
    s1_academic_data_visualization_details.append("Arial font in typography.md")

    # Check vector export rule
    export_md = REPO_ROOT / "references" / "export-specs.md"
    with open(export_md, encoding="utf-8", errors="replace") as f: export_src = f.read()
    s1_academic_data_visualization_passes.append("cairo_pdf" in export_src)
    s1_academic_data_visualization_details.append("cairo_pdf vector export")

    # 检查 450 dpi 栅格校样规则。
    s1_academic_data_visualization_passes.append("450" in export_src or "dpi = 450" in export_src.lower())
    s1_academic_data_visualization_details.append("450dpi rule")

    report["scenarios"]["S1_pca"] = {
        "academic-data-visualization": {
            "passed": sum(s1_academic_data_visualization_passes), "total": len(s1_academic_data_visualization_passes),
            "pass_rate": sum(s1_academic_data_visualization_passes) / len(s1_academic_data_visualization_passes),
            "checks": s1_academic_data_visualization_details,
        },
        "baseline": {
    ...
    for a in assets:
        path = REPO_ROOT / "assets" / "figures" / a
    ...
    for sid, data in sorted(report["scenarios"].items()):
        cn = data["academic-data-visualization"]
        bl = data["baseline"]
        total_cn += cn["total"]
        pass_cn += cn["passed"]
        total_bl += bl["total"]

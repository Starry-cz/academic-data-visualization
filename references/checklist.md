...
2. Color palette baseline must contain: `CATEGORICAL = ["#0072B2", "#009E73", "#E69F00", "#CC79A7", "#D55E00", "#56B4E9"]` AND `DIVERGING = ["#2166AC", "#F7F7F7", "#B2182B"]`
3. Export baseline 鈥?must contain: `pdf.fonttype: 42` AND `svg.fonttype: 'none'` AND a function named `save_nature_ready_figure`
...
**Fix if FAIL:** Replace with a perceptually uniform sequential colormap. For diverging data, use the Academic Data Visualization standard `#2166AC - #F7F7F7 - #B2182B`. For sequential data, use the shared blue sequential palette or a perceptually uniform single-hue alternative.
...
Pass 3 鈥?Visual Verification:
  ...
  [PASS] VV-3: All text legible at final size and in the 450dpi raster proof
  [WARN] VV-4: Baseline blue (#0072B2) vs bluish green (#009E73) distinct but check greyscale

...
```r
png("output.png", width=2.559, height=2.559, units="in", res=450, type="cairo")
print(plot)
dev.off()
```
...
```r
ggsave("output.pdf", plot, width=183, height=85, units="mm", device=cairo_pdf)
```

## 2. Turn OFF showtext_auto() before png()

showtext converts text glyphs to vector paths. In PDF output (vector) this is fine — paths scale infinitely. In PNG output (raster), paths can render poorly even at 450 dpi on a wide canvas. Cairo's native font renderer produces smooth anti-aliased glyphs.
...
Never hardcode `width=4, height=4` in an R `png()` call. Width and height MUST match the panel spec from the composition engine. For a 65mm panel at 450dpi:

```python
# Python — compose.py provides this
spec = {"width_mm": 65, "height_mm": 65, "dpi": 450}
r_call = compose.r_png_device(spec, "panel_output.png")
# → png("panel_output.png", width=2.559, height=2.559, units="in", res=450, type="cairo")
```

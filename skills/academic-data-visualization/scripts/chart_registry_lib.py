...
VERIFIED_TEMPLATES_DIR = ROOT / "templates" / "production-verified"
...
def production_records(registry: dict[str, Any]) -> list[dict[str, Any]]:
    return [chart for chart in registry["charts"] if chart["implementation_status"] == "production_verified"]


def asset_records(registry: dict[str, Any]) -> list[dict[str, Any]]:
    """Return records with a real checked-in implementation, regardless of verification tier."""
    return [chart for chart in registry["charts"] if chart["asset_path"] is not None]

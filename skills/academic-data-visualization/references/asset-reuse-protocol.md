...
4. If the status is `production_verified + release_passed`, read `directory-map.md` and follow the declared `asset_path`.
5. Open Manifest v2, the entrypoint, fixture, verification record, example QA report, and companion preview before deciding to reuse it.
6. If the status is `pattern`, `legacy_example`, or `none`, do not invent or upgrade an asset path; classify the work as visual adaptation or new implementation.
...
4. Execute through `scripts/run_asset.py`; verify that all expected outputs exist, open successfully, and pass the stored output contract.
